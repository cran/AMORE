...
/******************************************************************************************************************/
SEXP ADAPTgdwm_Forward_MLPnet (SEXP net, SEXP Pvector, SEXP rho);
SEXP ADAPTgdwm_Backwards_MLPnet (SEXP net, SEXP target, SEXP rho);
/******************************************************************************************************************/
/*   MLPneuron Level */
/******************************************************************************************************************/
SEXP ADAPTgdwm_Forward_MLPneuron(SEXP net, SEXP ind_neuron, SEXP rho);
SEXP ADAPTgdwm_Backwards_MLPneuron(SEXP net, SEXP ind_neuron, SEXP rho) ;
/******************************************************************************************************************/
...
SEXP this_neuron;
   ...
   for ( ind_layer=1; ind_layer < LENGTH(NET_LAYERS); ind_layer++ ) {
      for ( ind_neuron=0; ind_neuron < LENGTH( VECTOR_ELT(NET_LAYERS, ind_layer) ) ; ind_neuron++ ) {
         ...
         ADAPTgdwm_Forward_MLPneuron(net, this_neuron, rho);
      }
   }
   UNPROTECT(1);
   return(net);
...
SEXP this_neuron;
   ...
   for ( ind_layer=-1+LENGTH(NET_LAYERS); ind_layer>0; ind_layer-- ) {
      for ( ind_neuron=-1+LENGTH( VECTOR_ELT(NET_LAYERS, ind_layer) ); ind_neuron >=0;  ind_neuron-- ) {
         INTEGER(this_neuron)[0] = INTEGER(VECTOR_ELT(NET_LAYERS, ind_layer))[ind_neuron];
         ADAPTgdwm_Backwards_MLPneuron(net, this_neuron, rho );
      }
   }
   UNPROTECT(1);
   return(net);
...
/******************************************************************************************************************/
   ...
   /* PROTECT(arg3  = duplicate(NET_OTHER_ELEMENTS) ); n_protected++;      */
      ...
      for ( ind_other_neuron=0; ind_other_neuron < LENGTH(OUTPUT_LINKS) ; ind_other_neuron++ ) {
         ...
         aux_DELTA  += REAL(VECTOR_ELT(VECTOR_ELT(NET_NEURONS, that_neuron), id_WEIGHTS))[that_aim] * REAL(VECTOR_ELT(VECTOR_ELT(VECTOR_ELT(NET_NEURONS, that_neuron),id_METHOD_DEP_VARIABLES),id_ADAPTgdwm_DELTA))[0];
       }
   }

   REAL(ADAPTgdwm_DELTA)[0] = aux_DELTA * REAL(V1)[0];
   bias_change    = REAL(ADAPTgdwm_MOMENTUM)[0] * REAL(ADAPTgdwm_FORMER_BIAS_CHANGE)[0] -  REAL(ADAPTgdwm_LEARNING_RATE)[0] * REAL(ADAPTgdwm_DELTA)[0];
   REAL(ADAPTgdwm_FORMER_BIAS_CHANGE)[0] = bias_change;
   ...
   for (ind_weight = 0; ind_weight < LENGTH(WEIGHTS); ind_weight++) {
      ...
      weight_change  = REAL(ADAPTgdwm_MOMENTUM)[0] * REAL(ADAPTgdwm_FORMER_WEIGHT_CHANGE)[ind_weight] - REAL(ADAPTgdwm_LEARNING_RATE)[0] * REAL(ADAPTgdwm_DELTA)[0] * x_input ;
      REAL(WEIGHTS)[ind_weight] += weight_change;
      REAL(ADAPTgdwm_FORMER_WEIGHT_CHANGE)[ind_weight] = weight_change;
   }
