...
SEXP BATCHgd_loop_MLPnet (SEXP origNet, SEXP Ptrans, SEXP Ttrans, SEXP nepochs, SEXP rho) {
   ...
   for (epoch=0; epoch < n_epochs; epoch++) {
         ...
         /**/
            if (ptneuron->type==TYPE_OUTPUT) {
               ...
               for ( ind_other_neuron=0; ind_other_neuron <= ptneuron->last_output_link ; ind_other_neuron++ ) {
                  ...
                  aux_DELTA     += pt_that_neuron->method_dep_variables.batchgd.delta * pt_that_neuron->weights[that_aim] ;
               }
            }
            ptneuron->method_dep_variables.batchgd.delta = aux_DELTA * ptneuron->v1;
            ...
            for (ind_weight = 0; ind_weight <= ptneuron->last_input_link; ind_weight++) {
               ...
               ptneuron->method_dep_variables.batchgd.sum_delta_x[ind_weight] += ptneuron->method_dep_variables.batchgd.delta * x_input ;
            }
            ptneuron->method_dep_variables.batchgd.sum_delta_bias += ptneuron->method_dep_variables.batchgd.delta ;
      ...
      for (ind_neuron=0; ind_neuron <= ptnet->last_neuron ; ind_neuron++ ) {
         ptneuron = ptnet->neurons[ind_neuron];
         bias_change = - ptneuron->method_dep_variables.batchgd.learning_rate * ptneuron->method_dep_variables.batchgd.sum_delta_bias;
         ptneuron->bias += bias_change;
         for (ind_weight = 0; ind_weight <= ptneuron->last_input_link; ind_weight++) {
            weight_change  =  - ptneuron->method_dep_variables.batchgd.learning_rate * ptneuron->method_dep_variables.batchgd.sum_delta_x[ind_weight] ;
