...
SEXP BATCHgdwm_loop_MLPnet (SEXP origNet, SEXP Ptrans, SEXP Ttrans, SEXP nepochs, SEXP rho) {
   ...
   for (epoch=0; epoch < n_epochs; epoch++) {
      for (fila=0, Pcounter=0, Tcounter=0; fila < Ptransdim[1]; fila++) {
         ...
         /* END batchgdwm_forward_MLPneuron */
         }
         /* END     void batchgdwm_forward_mlpnet(AMOREnet * ptnet)   */


         /* BEGIN   void Parcial_batchgdwm_backwards_MLPnet (AMOREnet * ptnet, SEXP rho) */
         ...
         /**/
            if (ptneuron->type==TYPE_OUTPUT) {
               ...
               for ( ind_other_neuron=0; ind_other_neuron <= ptneuron->last_output_link ; ind_other_neuron++ ) {
                  ...
                  aux_DELTA     += pt_that_neuron->method_dep_variables.batchgdwm.delta * pt_that_neuron->weights[that_aim] ;
               }
            }
            ptneuron->method_dep_variables.batchgdwm.delta = aux_DELTA * ptneuron->v1;
            ...
            for (ind_weight = 0; ind_weight <= ptneuron->last_input_link; ind_weight++) {
               ...
               ptneuron->method_dep_variables.batchgdwm.sum_delta_x[ind_weight] += ptneuron->method_dep_variables.batchgdwm.delta * x_input ;
            }
            ptneuron->method_dep_variables.batchgdwm.sum_delta_bias += ptneuron->method_dep_variables.batchgdwm.delta ;
      ...
      for (ind_neuron=0; ind_neuron <= ptnet->last_neuron ; ind_neuron++ ) {
         ptneuron = ptnet->neurons[ind_neuron];
         bias_change = ptneuron->method_dep_variables.batchgdwm.momentum * ptneuron->method_dep_variables.batchgdwm.former_bias_change - ptneuron->method_dep_variables.batchgdwm.learning_rate * ptneuron->method_dep_variables.batchgdwm.sum_delta_bias;
         ptneuron->method_dep_variables.batchgdwm.former_bias_change = bias_change ;
         ptneuron->bias += bias_change;
         for (ind_weight = 0; ind_weight <= ptneuron->last_input_link; ind_weight++) {
            weight_change  =  ptneuron->method_dep_variables.batchgdwm.momentum * ptneuron->method_dep_variables.batchgdwm.former_weight_change[ind_weight] - ptneuron->method_dep_variables.batchgdwm.learning_rate * ptneuron->method_dep_variables.batchgdwm.sum_delta_x[ind_weight] ;
